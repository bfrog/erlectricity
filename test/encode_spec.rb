...
context "When packing to a binary stream" do
  ...
  specify "A symbol should be encoded to an erlang atom" do
    ...
    assert_equal write_any(:haha), get_erl_with_magic("haha")
  end
  ...
  specify "A boolean should be encoded to an erlang atom" do
    ...
    assert_equal( write_any(true), get_erl_with_magic("true"))
    assert_equal( write_any(false), get_erl_with_magic("false"))
  end

  specify "A number should be encoded as an erlang number would be" do
    #SMALL_INTS
    get{@encoder.write_fixnum 0}.should == get_erl("0")
    assert_equal( get{@encoder.write_fixnum 255}, get_erl("255"))
    assert_equal( write_any(0), get_erl_with_magic("0"))
    assert_equal( write_any(255), get_erl_with_magic("255"))

    #INTS
    assert_equal( get{@encoder.write_fixnum 256}, get_erl("256"))
    assert_equal( get{@encoder.write_fixnum((1 << 27) - 1)}, get_erl("#{(1 << 27) - 1}"))
    assert_equal( get{@encoder.write_fixnum(-1)}, get_erl("-1"))
    assert_equal( get{@encoder.write_fixnum(-(1 << 27))}, get_erl("#{-(1 << 27)}"))
    assert_equal( write_any(256), get_erl_with_magic("256"))
    assert_equal( write_any((1 << 27) - 1), get_erl_with_magic("#{(1 << 27) - 1}"))
    assert_equal( write_any(-1), get_erl_with_magic("-1"))
    assert_equal( write_any(-(1 << 27)), get_erl_with_magic("#{-(1 << 27)}"))
  ...
  specify "An Erlectiricity::NewReference should encode back to its original form" do
    ...
    assert_equal( write_any(ruby_ref), ref_bin.bytes.to_a)
  end
  ...
  specify "An Erlectiricity::Pid should encode back to its original form" do
    ...
    assert_equal( get{@encoder.write_pid(ruby_pid)}, pid_bin[1..-1].bytes.to_a)
    assert_equal( write_any(ruby_pid), pid_bin.bytes.to_a)
  end
  ...
  specify "An array written with write_tuple should encode as erlang would a tuple" do
    ...
    assert_equal( get{@encoder.write_tuple [3] * 255}, get_erl("{#{([3] * 255).join(',')}}"))
    assert_equal( get{@encoder.write_tuple [3] * 256}, get_erl("{#{([3] * 256).join(',')}}"))
    assert_equal( get{@encoder.write_tuple [3] * 512}, get_erl("{#{([3] * 512).join(',')}}"))
  end

  specify "An array should by default be written as a tuple" do
    assert_equal( write_any([1,2,3]), get_erl_with_magic("{1,2,3}"))
    assert_equal( write_any([3] * 255), get_erl_with_magic("{#{([3] * 255).join(',')}}"))
    assert_equal( write_any([3] * 256), get_erl_with_magic("{#{([3] * 256).join(',')}}"))
    assert_equal( write_any([3] * 512), get_erl_with_magic("{#{([3] * 512).join(',')}}"))
  end

  specify "An Erlectricity::List should by default be written as a list" do
    assert_equal( write_any(Erl::List.new([1,2,300])), get_erl_with_magic("[1,2,300]"))
    assert_equal( write_any(Erl::List.new([300] * 255)), get_erl_with_magic("[#{([300] * 255).join(',')}]"))
    assert_equal( write_any(Erl::List.new([300] * 256)), get_erl_with_magic("[#{([300] * 256).join(',')}]"))
    assert_equal( write_any(Erl::List.new([300] * 512)), get_erl_with_magic("[#{([300] * 512).join(',')}]"))
  end
  ...
  specify "An array written with write_list should encode as erlang would a list" do
    ...
    assert_equal( get{@encoder.write_list [300] * 255}, get_erl("[#{([300] * 255).join(',')}]"))
    assert_equal( get{@encoder.write_list [300] * 256}, get_erl("[#{([300] * 256).join(',')}]"))
    assert_equal( get{@encoder.write_list [300] * 256}, get_erl("[#{([300] * 256).join(',')}]"))
  end
  ...
  specify "a string should be encoded as a erlang binary would be" do
    ...
    assert_equal( write_any("hey who"), get_erl_with_magic("<< \"hey who\" >>"))
    assert_equal( write_any(""), get_erl_with_magic("<< \"\" >>"))
  end
